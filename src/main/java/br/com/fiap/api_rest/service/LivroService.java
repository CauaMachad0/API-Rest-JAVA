...
public class LivroService {
    ...
    @Autowired
    private AutorRepository autorRepository; // Corrigido para o tipo AutorRepository

    public Livro requestToLivro(LivroRequest livroRequest) {
        Livro livro = new Livro();

        List<Autor> autores = new ArrayList<>();
        for (String autorNome : livroRequest.getAutores()) {
            Autor autor = autorRepository.findByNome(autorNome); // Método findByNome no AutorRepository
            if (autor != null) {
                autores.add(autor);
            }
        }
    ...
    public Livro recordToLivro(LivroRequestDTO livroRecord) {
        ...
        livro.setAutores(livroRecord.autores()); // Presumo que livroRecord.autores() seja uma lista de autores
        return livro;
    }
    ...
    public List<LivroResponse> livrosToResponse(List<Livro> livros) {
        ...
        livros.forEach(livro -> listaLivros.add(livroToResponse(livro)));
        return listaLivros;
    }
